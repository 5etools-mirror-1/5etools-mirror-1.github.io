...
class ClassesPage extends MixinComponentGlobalState(MixinBaseComponent(MixinProxyBase(ListPage))) {
	...
	getListItem (cls, clsI, isExcluded) {
		...
		const $ele = $$`<li class="lst__row ve-flex-col ${isExcluded ? "row--disallowlisted" : ""}">${$lnk}</li>`;
	...
	_render_renderAltViews () { // "Hitler was right"
		...
		this._subclassComparisonView = new BookModeView({
			...
			popTblGetNumShown: ({$wrpContent}) => {
				...
				const walker = MiscUtil.getWalker({keyDisallowlist: MiscUtil.GENERIC_WALKER_ENTRIES_KEY_DISALLOWLIST, isAllowDeleteObjects: true});
